...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await prisma.googleTrendJob.findFirst({
      where: {
    ...
    if (existing) {
      return NextResponse.json(
        { error: "Es laeuft bereits ein Google Trends Job", job: existing },
        { status: 409 }
      );
    }
    ...
    const job = await prisma.googleTrendJob.create({
      data: {
        userId: session.user.id,
        property,
    ...
    console.error("Error creating Google Trends job:", error);
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Error fetching Google Trends job:", error);

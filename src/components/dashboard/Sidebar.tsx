...
function NavigationItems({ 
  ...
  return (
    <ul role="list" className="space-y-1">
      {items.map((item) => {
        const isActive = pathname === item.href || (item.children && item.children.some(child => pathname === child.href || pathname.startsWith(item.href + "/")));
...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    [...dataNavigation, ...reportingNavigation, ...processNavigation, ...planungNavigation].forEach(item => {
      if (item.children) {
        const isChildActive = item.children.some(child => pathname === child.href || (child.href !== item.href && pathname.startsWith(child.href + "/")));
      ...
      <div className="flex grow flex-col gap-y-5 overflow-y-auto bg-slate-50 dark:bg-slate-800 px-6 pb-4 border-r border-slate-200 dark:border-slate-700">
        ...
        <nav className="flex flex-1 flex-col">
          ...
          <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
            ...
            {hasFullAdminRights(session?.user?.role) && (
              <Link
                href="/content"
                className={`group flex gap-x-3 rounded-lg p-3 text-sm font-medium transition-all duration-200 ${
                  isContentActive
